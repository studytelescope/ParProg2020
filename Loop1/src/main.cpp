...
void calc(double* arr, uint32_t ySize, uint32_t xSize, int rank, int size)
{
	...
	if (rank == 0)
	{
		...
		{
			 uint32_t start_point = i * block_size;
			uint32_t end_point = start_point + block_size;
	...
	for (uint32_t i = 0; i < (end_point - start_point) * xSize; i++)
	{
	...
	if (rank == 0)
		for (int process_rank = 1; process_rank < size; process_rank++)
		{
			...
			start_point = process_rank * block_size;
			end_point = start_point + block_size;

			if(end_point > ySize) end_point = ySize;
			if(start_point > ySize) start_point = ySize;

			MPI_Recv (&(arr[start_point * xSize]), xSize * (end_point - start_point), MPI_DOUBLE, process_rank, 0, MPI_COMM_WORLD, &status);
		}
	else
	{
		MPI_Send(arr, xSize * (end_point - start_point), MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
		free(arr);
	}
}
